...
### 4. 容器与卡片文本换行安全规范（Card Boundary & Text Auto-Wrap Safety）
- **杜绝单行溢出越界**：卡片、色块容器及表格单元格内的文字，**严禁超出容器物理边界**。绝对禁止出现整行长文本不换行导致向右穿透、溢出到相邻卡片或画外的现象；
- **SVG 排版断行标准**：
  - SVG 源码编写卡片内多字描述时，必须根据容器可用宽度（`card_width - 2 * padding`）严格测算字符数；
  - 严禁在单行 `<text>` 中塞入超过卡片可用宽度的文本；
  - 超过单行宽度时，必须使用 `<tspan x="..." dy="...">`（或分行 `<text>`）进行显式折行；
  - 窄卡片（如 5 列布局、宽 ≤ 230px）内描述文字字号推荐 `14~15px`，单行最多容纳 12~14 个中文字符，长句须折为 2~3 行；
- **PPTX 原生自适应换行安全网**：
  - 所有生成的 PPTX 文件，所有包含文本框的形状一律强制启用原生自动换行：`shape.text_frame.word_wrap = True`；
  - 文本框左右边距（`margin_left`/`margin_right`）与卡片对齐，即便在不同操作系统、Office/WPS 渲染引擎或 DPI 缩放下，文本也会自动在容器边界内软换行，彻底杜绝文字被横向截断或遮挡。

### 5. 宏观部分/主章节转场过渡页强制门禁（Mandatory Section Transition Slides）
- **多部分强制转场原则**：
  - 只要演示文稿包含 2 个及以上的宏观“部分（Part）”、“大模块”或“小节/主章节（Chapter/Section）”，在大纲规划与页面制作时，**必须强制在每个部分开始前插入 1 页独立的章节展示/转场过渡页（Section Transition / Divider Slide）**；
  - **严禁偷工减料**：绝对禁止在目录页之后直接跳入正文内容页，绝对禁止仅依靠正文内容页左上角的小字标签充当转场；
- **固定版式标准（继承 P05 规范）**：
  - **左侧浅底面板**（宽约 340~520px，填满画布全高，左侧贯穿 12~14px 垂直主色条）；
  - **主视觉超大粗体数字**（`200px` / `Pt(96~110)` 加粗，色彩呼应主题）；
